...
def multiline_text(text: str, x_start: int, y_start: int, text_size: float, width: int):
    ...
    for line in text.split('\n'):
        for word in line.split():
            ...
            word += ' '
            if line.startswith('  ') and (x == x_start):
                leading_spaces = (len(line) - len(line.strip())) - 1
...
def draw_badge():
    ...
    display.text(title, ((TEXT_WIDTH - title_text_width) // 2), (TITLE_HEIGHT // 2) + 2, WIDTH, TITLE_TEXT_SIZE)

    # Draw the name, scaling it based on the available width
    display.set_pen(15)
    # display.set_font("bitmap14_outline")
    display.set_font("sans")
    multiline_text(NAME, LEFT_PADDING, TITLE_HEIGHT + 14, NAME_TEXT_SIZE, WIDTH)
